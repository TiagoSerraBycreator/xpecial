...
async function checkCompanyData() {
  try {
    ...
    if (company) {
      ...
      console.log('\n=== VAGAS ===')
      company.jobs.forEach((job: any, index: number) => {
        ...
        const statusCount = job.applications.reduce((acc: Record<string, number>, app: any) => {
    ...
    if (company) {
      ...
      const statusCount = currentMonthApplications.reduce((acc: Record<string, number>, app: any) => {
        acc[app.status] = (acc[app.status] || 0) + 1
        return acc
      }, {} as Record<string, number>)
